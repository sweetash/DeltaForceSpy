...
def price_check_flow(card_info):
    ...
    max_price = card_info.get('base_price') * 1.1
    ...
    if premium < 0 or current_price < ideal_price or current_price - base_price <= 100:
        if card_info.get('buyMax', 0) == 1:
            ...
            pyautogui.moveTo(screen_width*0.9104, screen_height*0.7807)  # 点击 Max 按钮
            pyautogui.click()
            time.sleep(0.02)
...
def all_cards_completed(cards_to_monitor):
    """检查所有卡片是否达到购买上限"""
    max_buy_limit = 2  # 与 price_check_flow 中的上限一致
    return all(card.get('buy_count', 0) >= max_buy_limit for card in cards_to_monitor)
